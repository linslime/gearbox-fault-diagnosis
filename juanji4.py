...
def xs_gen(path=MANIFEST_DIR, batch_size=Batch_size, train=True, Lens=Lens):
    ...
    for n in range(5):
        ...
        for i in range(29):
            ...
            for j in range(i * 900, i * 900 + 900 + 1):
...
def build_model(input_shape=(TIME_PERIODS,), num_classes=5):
    ...
    model.add(Conv1D(64, 8, strides=1, activation='relu', input_shape=(TIME_PERIODS, 1)))
    model.add(Conv1D(64, 8, strides=1, activation='relu', padding="same"))
    model.add(MaxPooling1D(2))

    model.add(Conv1D(256, 4, strides=1, activation='relu', padding="same"))
    model.add(Conv1D(256, 4, strides=1, activation='relu', padding="same"))
    model.add(MaxPooling1D(2))
    ...
    model.add(Conv1D(1024, 2, strides=1, activation='relu', padding="same"))
    model.add(Conv1D(1024, 2, strides=1, activation='relu', padding="same"))
    model.add(MaxPooling1D(2))
