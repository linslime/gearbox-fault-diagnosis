...
def xs_gen(path=MANIFEST_DIR, batch_size=Batch_size, train=True, Lens=Lens):
    ...
    for n in range(5):
        pass
        df = pd.read_excel(path, sheet_name=tablename[n], names=['value'], usecols=[3])
        data = df.values
        # print(data)
        for i in range(29):
            temp = []
...
def build_model(input_shape=(TIME_PERIODS,), num_classes=5):
    ...
    model.add(Conv1D(64, 8, strides=1, activation='tanh', input_shape=(TIME_PERIODS, 1)))
    model.add(Conv1D(64, 8, strides=1, activation='tanh', padding="same"))
    ...
    model.add(Conv1D(512, 4, strides=1, activation='tanh', padding="same"))
    model.add(Conv1D(512, 4, strides=1, activation='tanh', padding="same"))
    model.add(MaxPooling1D(2))
